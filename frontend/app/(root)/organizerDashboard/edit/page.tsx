...
import { useEventDetailsQuery, useUpdateEventMutation } from '@/redux/features/eventApislice'
        ...
        if(data?.image){
           const convertImage = async ()=>{
              try{
                ...
                const fileName =  data.image.split('/').pop() || 'edit-event';
                const file = new File([blob],fileName,{type:blob.type})
                form.setValue("imageFile",file)
              }catch(error){
                console.error(`Error fetching image ${data.image}:`, error)
              }
           }
           convertImage()
        }
                ...
                <form onSubmit={form.handleSubmit(onSubmit)}
                  ...
                  <div className='w-full flex justify-end min-h-[15vh] gap-x-4 mt-7'>
                    ...
                    }
                    {
                      steps === 4 ? (
                        <Button type="submit" 
                          className='bg-backgroundNavyBlue hover:bg-backgroundNavyBlue/80' disabled={isLoading}>
                           {
